...
(defun write-extended-executes-test (&optional command path-to-shell option)
  "Write `py-execute-block...' etc. "
  (interactive)
  ;; (load-shells)
  (let ((py-bounds-command-names (if command (list command) py-bounds-command-names))
  ...
  (py-bug-tests-intern 'py-execute-" ele "-" elt)))
	  ...
	  (if (string= "" elt)
	    ...
	    (insert (concat "(set-buffer buffer)(goto-char (point-min))(sit-for 0.1 t)(search-forward \"the py-execute-" ele "-" elt))
	    (unless (string= pyo "")(insert (concat "-" pyo)))
...
(defun write-extended-executes-test-2 (&optional command path-to-shell option)
  "Write `py-execute-block...' etc. "
  (interactive)
  ;; (load-shells)
  (let ((py-bounds-command-names (if command (list command) py-bounds-command-names))
  ...
  (assert (search-forward \"the py-execute-" ele "-" elt))
	    (unless (string= pyo "")(insert (concat "-" pyo)))
