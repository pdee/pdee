...
(defun write-shell-ert-tests (&optional command path-to-shell option)
  "Write `py-shell...' etc. "
  (interactive)
  (set-buffer (get-buffer-create "py-shell-ert-tests.el"))
  (erase-buffer)
  (switch-to-buffer (current-buffer))
  (insert ";;; py-shell-ert-tests.el --- py-shell ert tests\n")
  (insert arkopf)
  (dolist (ele py-shells)
    ...
    (let ((buffer (prin1-to-string (py--choose-buffer-name ele)))
      ...
      (insert (make-string 2 ?\ )) 
      (insert "(should (< 99 comint-last-input-end)))\n\n")
      ))
  (insert "(provide 'py-shell-ert-tests)
;;; py-shell-ert-tests.el ends here\n ")
  (emacs-lisp-mode)
  (write-file (concat py-install-directory "/test/py-shell-ert-tests.el"))
  (switch-to-buffer (current-buffer))
  )
