;;; python-mode-utils.el --- generate parts of python-mode.el  -*- lexical-binding: t; -*-
...
(defun py-write-file-forms ()
  ...
  (interactive \"fFile: \")
  (let ((buffer (py-shell nil nil nil \"" ele "\" nil t)))
  ...
  (interactive \"fFile: \")
  (let ((buffer (py-shell nil nil t \"" ele "\" nil t)))
    (py--execute-file-base filename (get-buffer-process buffer) nil buffer nil t)))\n\n")))
...
(defun write--unified-extended-execute-forms-docu (ele elt pyo)
  ...
  (cond
   ((string-match "" elt)
    (insert (concat " a " py-python-command)))
...
(setq py--basic-execute-forms "(defun py--execute-prepare (form shell \&optional dedicated switch beg end filename fast proc wholebuf split)
  \"Update some vars\.\"
  (save-excursion
...
Return outmost indentation reached.\"
  (interactive \"\*P\")
  (py--shift-forms-base \"" ele "\" (or arg py-indent-offset)))
...
(defun py-write-up-forms ()
  (interactive)
  (set-buffer (get-buffer-create "python-components-up.el"))
  (erase-buffer)
  (when (called-interactively-p 'interactive)
    (switch-to-buffer (current-buffer)))
  (insert ";;; python-components-up.el -- Searching upwards in buffer -*- lexical-binding: t; -*- \n")
  (insert arkopf)
  (dolist (ele py-down-forms)
...
\(provide 'python-components-up)")
  (when (called-interactively-p 'interactive) (switch-to-buffer (current-buffer))
        (emacs-lisp-mode))
  (write-file (concat components-directory "/python-components-up.el")))

(defun py-write-down-forms ()
  (interactive)
  (set-buffer (get-buffer-create "python-components-down.el"))
  (erase-buffer)
  (when (called-interactively-p 'interactive)
    (switch-to-buffer (current-buffer)))
  (insert ";;; python-components-down.el -- Searching downwards in buffer -*- lexical-binding: t; -*- \n")
  (insert arkopf)
  ;; down
  (dolist (ele py-down-forms)
    (if (string= "statement" ele)
...
\(provide 'python-components-down)")
  (when (called-interactively-p 'interactive) (switch-to-buffer (current-buffer))
        (emacs-lisp-mode))
  (write-file (concat components-directory "/python-components-down.el")))
...
Use backward-statement for ‘top-level’, also bol-forms do not make sense here"
  (interactive)
...
")
  ...
  ;; do not handle (partial)-expression forms here
...
Stores data in kill ring\"
  ...
  (let ((erg (py--mark-base \"" ele "\")))
    ...
    ;; expression-forms do not make sense WRT bol
    (dolist (ele py-bol-forms)
      (insert (concat "
\(defun py-kill-" ele " ()
  \"Delete " ele " at point.
