...
$EMACS -Q --batch --eval "(message (emacs-version))" --eval "(add-to-list 'load-path \"$PDIR/\")" --eval "(add-to-list 'load-path \"$TESTDIR/\")" -load $SETUP -load $PYTHONMODE -l $TEST2 -l $TEST4 -l $TEST5 -l $TEST6 -l $TEST7 -l $TEST8 -l $TEST10 -l $TEST11 -f ert-run-tests-batch-and-exit
